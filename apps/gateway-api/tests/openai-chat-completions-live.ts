...
const payloadPath = join(__dirname, "fixtures", "sample-payload.json");
...
export function getProviderIds(): string[] {
  ...
  const providerIds = Object.keys(openaiCompatibleProviders);
...
function buildModel(providerId: string): string {
  const provider = openaiCompatibleProviders[providerId];
