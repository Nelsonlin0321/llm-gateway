...
- Pipeline modules:
  ...
  - `src/load/` — transactional inserts; auto-create daily partitions on miss
...
- `request_log` / `event_log` are `PARTITION BY RANGE (log_date)`. On
  `no partition of relation … found for row`, create
  `{table}_{YYYY_MM_DD}` for that day and retry once (`src/load/partitions.ts`).
