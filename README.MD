...
Size figures are from the current `gateway-api` on Bun, measured locally. Added latency is from the same machine: the real Hono proxy in front of a local mock, minus a direct call to that mock. Redis, Postgres, and the upstream model are not in that number.
...
| Added latency | p50 **0.16 ms**, p99 **0.30 ms** |
...
The script is [`apps/gateway-api/scripts/measure-gateway-overhead.ts`](apps/gateway-api/scripts/measure-gateway-overhead.ts) (`bun run bench:overhead` from `apps/gateway-api`). Both sides send the same non-streaming chat body. The gateway side is the real OpenAI proxy: Hono middleware, an in-memory HS256 verify, two AES-256-GCM decrypts, payload parse, model rewrite, `hono/proxy`, and JSON response capture. Provider resolution is a fixed in-memory record. Nothing connects to Redis or Postgres.

300 samples after 50 warmup pairs, on this machine:

| Path | p50 | p90 | p99 |
| --- | --- | --- | --- |
| Client → mock | 0.048 ms | 0.081 ms | 0.754 ms |
| Client → gateway → mock | 0.212 ms | 0.302 ms | 0.434 ms |
| **Overhead (paired)** | **0.163 ms** | **0.220 ms** | **0.302 ms** |

The homepage rounds that overhead to **0.16 ms** at p50 and **0.30 ms** at p99.

Streaming still copies chunks through as they arrive. Non-streaming JSON is buffered until the mock body finishes, which is included in the times above. Each in-flight stream keeps a copy of the SSE text, capped at 1,000,000 characters. The request-log `XADD` is stubbed in this run, so a Redis write is not included.
