...
| [`gateway-ingest`](./apps/gateway-ingest) | Stream → Postgres | Cron | Cloudflare Worker (`scheduled()`) |
...
- **Redis** (Upstash HTTP REST for Workers; cache + request-log stream)
...
bun install
# REDIS_URL (Upstash) + DATABASE_URL
